...
const HorizondalScroll = () => {
 useEffect(() => {
  ...
  const cardPositions = [
    ...
    const imageContainer = document.querySelector(".threedimages") as Element;

    for (let i = 1; i <= 10; i++) {
      const card = document.createElement("div");
      card.className =
        `threedcard threedcard-${i} absolute w-[250px] h-[250px] rounded-[2em] preserve-3d will-change-transform overflow-hidden`;
    ...
    const cards = document.querySelectorAll(".threedcard");
    cards.forEach((card) => {
      gsap.set(card, {
        z: -5000,
        scale: 0,
